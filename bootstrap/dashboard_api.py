...
def _media_asset_content_recovery_preflight_url(asset_id: str) -> str:
    encoded = urllib.parse.quote(asset_id, safe="")
    return f"/api/dashboard/media-assets/content-recovery/preflight?asset_id={encoded}"
...
def _fetch_media_assets_from_runtime(params: Mapping[str, str]) -> list[dict[str, Any]]:
    ...
    for item in data:
        ...
        if asset_id and not _first_text(asset.get("content_recovery_preflight_url")):
            asset["content_recovery_preflight_url"] = (
                _media_asset_content_recovery_preflight_url(asset_id)
            )
        assets.append(asset)
    return assets
...
def create_dashboard_app(
    ...
    *,
    ...
    @app.get("/api/dashboard/media-assets/content-recovery/preflight")
    def get_dashboard_media_asset_content_recovery_preflight(
        asset_id: str = Query(..., min_length=1),
        target_id: str = "",
        operator_id: str = "",
        approval_id: str = "",
        ...
        for key, value in {
            ...
            if str(value or "").strip():
                params[key] = str(value).strip()
        url = (
            f"{runtime_base_url}/v1/media-assets/content-recovery/preflight?"
        ...
        try:
            ...
            detail = "agent-runtime media content recovery preflight 请求失败"
            try:
                raw = exc.read().decode("utf-8", errors="replace").strip()
            except OSError:
                raw = ""
            ...
            raise HTTPException(
                status_code=502,
                detail=f"agent-runtime media content recovery preflight 不可用: {exc}",
            ) from exc
        data = payload.get("data") if isinstance(payload, Mapping) else payload
        if not isinstance(data, Mapping):
            raise HTTPException(
                status_code=502,
                detail="agent-runtime media content recovery preflight 响应格式无效",
            )
        return dict(data)
