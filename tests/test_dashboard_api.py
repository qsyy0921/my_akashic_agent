...
def test_dashboard_media_asset_content_access_plan_proxy(
    tmp_path,
    monkeypatch,
) -> None:
    asset_id = "asset:qq:image:187890369:ready:1"
        ...
        return _fake_urlopen_response(
            {
                ...
                "data": {
                    ...
                    "asset_id": asset_id,
                    "content_endpoint": "/v1/media-assets/asset%3Aqq/content",
                    "side_effect": "none",
                },
            }
        )
    ...
    assert payload["reason"] == "media_asset_content_ready"
    assert payload["side_effect"] == "none"


def test_dashboard_media_asset_content_access_plan_preserves_runtime_http_error(
    tmp_path,
    monkeypatch,
) -> None:
    def _fake_urlopen(url, timeout=None):  # type: ignore[no-untyped-def]
...
def test_dashboard_media_asset_content_access_plan_reports_invalid_runtime_response(
    tmp_path,
    monkeypatch,
) -> None:
    def _fake_urlopen(_url, timeout=None):  # type: ignore[no-untyped-def]
