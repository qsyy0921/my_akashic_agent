...
- [ ] 查看 `/v1/media-assets/content-recovery/preflight?asset_id=...&operator_id=...&approval_id=...`：缺失/不可访问内容且 approval active 时应返回 `ready=true/reason=media_asset_content_recovery_preflight_ready/suggested_audit.status=planned`；ready 内容应返回 `media_asset_content_recovery_not_required`。调用后不得创建 approval/mutation、不得下载/恢复/缓存内容、不得打开内容流、不得触发 OCR/VLM/RAG/AI。
- [ ] 通过 `POST /v1/media-assets/content-recovery` 做一次 dry-run 和一次小范围 HTTP/HTTPS 正式恢复：dry-run 应返回 `side_effect=none` 且不写文件；正式恢复必须先有 active approval，成功后应写入本地 cache、更新 media registry `local_path/recovered_from_url/recovery_scope`，记录 applied control mutation audit，并让 `/v1/media-assets/{asset_id}/content` 可显示；整个过程不得触发 OCR/VLM/RAG/AI。
- [ ] 查看 `/v1/runtime-overview` 的 `Media Content Recovery` card、`media_asset_content_recovery_*` summary 和 detail：应只读汇总 `media_asset_content/recover_content` control mutation audits，failed audit 存在时 card 为 danger；调用 overview 不得创建 approval/mutation、不得调用 recovery executor、不得下载/缓存内容、不得触发 OCR/VLM/RAG/AI。
- [ ] 查看 runtime overview dashboard 的 `Media Content Recovery` detail：应显示 recovery audit totals、plan/preflight/recovery 链接和 recent mutation table，并保留 raw JSON；打开 detail 不得调用 preflight/recovery endpoint、不得创建 approval/mutation、不得下载/缓存内容、不得触发 OCR/VLM/RAG/AI。
- [ ] 查看 dashboard `/api/dashboard/media-assets/content-recovery/preflight?asset_id=...&operator_id=...&approval_id=...`：应与 Go `/v1/media-assets/content-recovery/preflight` 一致；消息列表/详情的 `media_assets` 应包含 `content_recovery_preflight_url`，但渲染列表/详情不得主动请求 preflight，不得创建 approval/mutation、下载/恢复/缓存内容或触发 AI。
- [ ] 查看 `/v1/media-assets/content-diagnostics?asset_id=...` 和 runtime overview dashboard 的 `Media Asset Content` detail：每个 asset 应包含 `content_recovery_preflight_endpoint` / preflight 链接；打开 detail 不得主动调用 preflight、不得创建 approval/mutation、下载/恢复/缓存内容或触发 AI。
...
- [ ] 查看 `/v1/runtime-overview` 的 `control_mutation_policy` card、`control_mutation_policy_*` summary 和 detail：应与 `/v1/control-mutations/policy` 一致，调用后不得创建 approval/mutation、不修改 env/config、不执行 cutover、不启动 worker、不 ack/nack MQ、不触发 AI。
- [ ] 查看 Python dashboard `/api/dashboard/runtime-overview` 的 `control_mutation_policy` card、summary 和 detail：应与 Go `/v1/runtime-overview` 一致，调用后不得创建 approval/mutation、不修改 env/config、不执行 cutover、不启动 worker、不 ack/nack MQ、不触发 AI。
...
- [ ] 查看 runtime overview dashboard 的 `Queue Topology` detail：应显示 provider/mode/phase、nodes/edges 和 work kind 表格，能直接看到 execution owner、ack owner、allowed 和 blockers；打开 detail 不得 publish/lease/ack/nack/term MQ，不得创建 AgentJob/outbox delivery，不得触发 Python AI。
