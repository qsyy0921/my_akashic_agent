...
| OI-005 | Media Assets | 媒体内容访问已可诊断和代理，但远程重新下载、文件恢复和内容缓存策略仍未统一。 | 图片/附件本地缺失时只能解释 unavailable，不能自动修复。 | 设计独立 media downloader/cache SDD；Go 管资产生命周期，Python 仍管 OCR/VLM/语义解析。 | Open |
...
| OI-008 | Frontend / Dashboard | runtime overview 已提供大量 card/detail，但部分 drilldown 仍依赖原始 JSON。 | 运维可读性不足，复杂状态需要人工读 payload。 | 优先消费现有 Go overview detail 做只读表格，不在 dashboard 加真实控制逻辑。 | Open |
