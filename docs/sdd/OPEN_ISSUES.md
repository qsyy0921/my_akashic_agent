...
| OI-005 | Media Assets | 媒体内容访问已可诊断、代理，已有 recovery plan、approval-bound preflight、dashboard 链接和 Go HTTP/HTTPS 本地 cache recovery executor；但 QQ/Telegram 私有源凭证、平台会话态重拉、自动后台重试和 Python AI enrichment 触发仍未统一。 | 对普通 HTTP/HTTPS 源，Go 已可在审批后下载缓存并修复本地访问；对需要平台 token/cookie/session 的源，仍可能需要 Python/provider-specific worker 或人工重新采集。 | 后续设计 provider-specific media fetch worker 与恢复后 AI enrichment job admission；Go 继续管资产生命周期、approval/audit/cache state，Python 管 OCR/VLM/语义解析和平台私有策略。 | Open |
...
| OI-008 | Frontend / Dashboard | runtime overview 已提供大量 card/detail，media asset content、media content recovery、queue topology、control audit 和 control mutation policy 已有表格化 drilldown，但部分其它 drilldown 仍依赖原始 JSON。 | 运维可读性仍有不足，复杂状态需要人工读 payload。 | 继续优先消费现有 Go overview detail 做只读表格，不在 dashboard 加真实控制逻辑。 | Open |
