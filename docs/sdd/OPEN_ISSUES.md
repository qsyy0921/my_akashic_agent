...
| OI-005 | Media Assets | 媒体内容访问已可诊断、代理，已有只读 recovery plan、approval-bound recovery preflight、dashboard preflight 链接和 diagnostics preflight endpoint，但远程重新下载、文件恢复和内容缓存 executor 仍未统一。 | 图片/附件本地缺失时可从前端和 runtime overview 看到恢复步骤、future executor scope 和 approval/audit 前置门禁，但不能自动修复。 | 设计独立 media downloader/cache executor SDD；Go 管资产生命周期、operator-approved executor 和审计，Python 仍管 OCR/VLM/语义解析。 | Open |
