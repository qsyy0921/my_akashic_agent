...
| OI-008 | Frontend / Dashboard | runtime overview 已提供大量 card/detail，media asset content、queue topology、control audit 和 control mutation policy 已有表格化 drilldown，但部分其它 drilldown 仍依赖原始 JSON。 | 运维可读性仍有不足，复杂状态需要人工读 payload。 | 继续优先消费现有 Go overview detail 做只读表格，不在 dashboard 加真实控制逻辑。 | Open |
