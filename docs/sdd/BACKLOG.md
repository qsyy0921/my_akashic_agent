...
- 若前端需要更强执行边界可视化，再把 execution owner、provider capability、external lease diagnostics 合并成独立 queue topology 面板；当前 `/v1/queue-backend`、runtime overview summary 和 dashboard control-plane detail 已提供只读诊断，避免在 dashboard 中加入真实控制逻辑。
...
- 若后续要做 Python worker 并发控制、autoscaling 或知识任务优先级调度，优先基于已落地的 Go `AgentJob` pressure、`Agent Job Worker Coverage` 和 `/v1/agent-job-capacity/plan`；当前只提供只读容量建议，不引入调度副作用。
- `Agent Job Capacity` 已进入 runtime overview 且 Python dashboard 已规范化 detail；后续如需前端 drilldown，可直接消费 dashboard detail，真实 autoscaling/优先级控制仍必须另做 operator ack 和审计设计。
- 后续若要把 capacity plan 推进成真实控制面，必须先补 operator ack、配置变更审计、限流/熔断策略和 Python worker 侧并发实现；不要直接让 Go 自动启动 AI worker 或修改模型/RAG/图片执行策略。
...
- knowledge planner cutover plan 已提供只读 enable/verify/rollback 步骤，Python dashboard 已规范化该 detail；若后续需要自动化 planner cutover，必须另加显式 operator ack、配置变更审计和回滚记录，不能让 plan endpoint 或 dashboard 自动改环境变量或启动 worker。
