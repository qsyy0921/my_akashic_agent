...
- 推进 QQ/NapCat Go adapter cutover：只读 `/v1/outbound-cutover/readiness` 和 `/v1/outbound-cutover/plan` 已完成；后续先做真实 live send smoke，再决定是否把 QQ channel alias 加入 Go outbound 或启用 Go local outbox worker。
