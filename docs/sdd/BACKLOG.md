...
- knowledge planner cutover plan 已提供只读 enable/verify/rollback 步骤；若后续需要自动化 planner cutover，必须另加显式 operator ack、配置变更审计和回滚记录，不能让 plan endpoint 自动改环境变量或启动 worker。
