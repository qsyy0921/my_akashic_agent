...
本文件记录后续候选任务和长期规划。进入具体迭代前，先从这里挑选本轮能完成的事项写入 `docs/sdd/TODO.md`；本轮未承诺的事项不要放进 TODO。已知缺口、风险、阻塞和待决策项统一记录在 `docs/sdd/OPEN_ISSUES.md`，不要混入 BACKLOG。
...
- 若前端需要更强队列或控制面审计可视化，再把 provider capability / control audit / control mutation policy detail 做成独立 dashboard 表格；当前 Go runtime overview 和 Python dashboard API 已提供 summary/card/detail，避免本轮扩大 UI 改造范围。
...
- 后续若要把 capacity / priority / cutover plan 推进成真实控制面，已有 operator approval ledger、approval check、control mutation preflight、control mutation policy API 和 control mutation audit ledger 可作为人工确认、校验和审计边界；下一步仍必须把具体 control-plane mutation 执行器强制绑定到 allowlisted target/action、approval id、mutation audit id、限流/熔断策略、回滚执行记录和 Python worker 侧并发实现。不要直接让 Go 自动启动 AI worker 或修改模型/RAG/图片执行策略。
...
- knowledge planner cutover plan 已提供只读 enable/verify/rollback 步骤，Python dashboard 已规范化该 detail；若后续需要自动化 planner cutover，必须将 mutation 显式绑定到 Go operator approval check 返回的有效记录和 control mutation audit 记录，不能让 plan endpoint 或 dashboard 自动改环境变量或启动 worker。
