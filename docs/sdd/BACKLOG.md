...
- 后续若要把 capacity / priority / cutover plan 推进成真实控制面，已有 operator approval ledger 与 approval check preflight 可作为人工确认和校验边界；下一步仍必须把具体 control-plane mutation 强制绑定到 approval id、配置变更审计、限流/熔断策略、回滚记录和 Python worker 侧并发实现。不要直接让 Go 自动启动 AI worker 或修改模型/RAG/图片执行策略。
...
- knowledge planner cutover plan 已提供只读 enable/verify/rollback 步骤，Python dashboard 已规范化该 detail；若后续需要自动化 planner cutover，必须将 mutation 显式绑定到 Go operator approval check 返回的有效记录、配置变更审计和回滚记录，不能让 plan endpoint 或 dashboard 自动改环境变量或启动 worker。
