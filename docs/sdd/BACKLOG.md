...
- 若后续要做 Python worker 并发控制、autoscaling 或知识任务优先级调度，优先基于已落地的 Go `AgentJob` pressure、`Agent Job Worker Coverage`、`/v1/agent-job-capacity/plan` 和 `/v1/agent-job-priority/plan`；当前只提供只读容量/优先级建议，不引入调度副作用。
