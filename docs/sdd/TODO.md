...
- [ ] 观察 proactive seen/rejection live 状态：主动推送候选流运行后确认 `.akashic-workspace/agent-runtime/proactive-state.json` 中出现 `seen_items` 或 `rejection_cooldowns`，并确认 SQLite fallback 不会放宽去重；如果 JSON store 过期记录增长明显，再做 Go-owned retention cleanup。
