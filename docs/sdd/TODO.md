...
- [ ] 补齐 `agent_job` NATS external lease 的剩余 cutover 能力：NATS 级 duplicate-delivery smoke、agent_job subject 订阅扩容、运行态执行 scope 显式开关，并评估是否需要安全的后台/启动 recover-expired 扫描。
