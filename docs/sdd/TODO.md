...
- [ ] 补 receiver lease 的重启恢复策略：当前 receiver status 已持久化并有 heartbeat，但 Telegram receiver lease 仍是内存态；如果只重启 Go，Python 侧续租会失败并不会自动重新 acquire。后续如需更强单实例 polling 保证，应增加 lease 持久化或 Python renew 失败后的 reacquire gate。
