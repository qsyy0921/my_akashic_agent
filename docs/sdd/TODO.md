...
- [ ] 配置当前运行态 `AKASHIC_ONEBOT_WS_URLS="qq=ws://127.0.0.1:3001,qq_2365524513=ws://127.0.0.1:3002"` 与 `AKASHIC_ONEBOT_ACCESS_TOKENS`，重启 `agent-runtime` 后通过日志、`GET /v1/delivery-adapters`、`GET /v1/runtime-workers` 和 runtime overview dashboard 确认 adapter/worker enabled。
