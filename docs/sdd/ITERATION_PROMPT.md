...
- 本轮结束前必须完成 TODO.md 中所有未完成项；不要只完成其中一个切片就停止。
...
- Python 做 AI runtime：模型/provider 路由、prompt/context、工具执行、Memory/RAG 抽取、chunking、embedding/rerank、检索策略、OCR/VLM、图片生成执行、群知识沉淀、评估脚本、策略实验和 provider-specific fallback。
...
- 每轮都要以“完成当前 TODO 全量内容”为交付单位；需要缩小范围时先改 TODO/SDD，不在实现后留下半成品。
```
