...
func TestRuntimeOverviewEndpointReturnsGoOwnedAggregate(t *testing.T) {
	...
	httptrigger.RegisterRuntimeOverviewRoutes(mux, staticRuntimeOverviewViewer{
		view: query.RuntimeOverviewView{
			Summary: map[string]any{
				...
				"knowledge_pipeline_rag_dataset_index_ready":            1,
				"knowledge_pipeline_rag_dataset_index_missing_snapshot": 1,
				"knowledge_pipeline_rag_dataset_index_empty":            1,
				"knowledge_pipeline_rag_dataset_index_lagging":          1,
	...
	if !bytes.Contains(response.Body.Bytes(), []byte(`"knowledge_pipeline_targets":2`)) ||
		...
		!bytes.Contains(response.Body.Bytes(), []byte(`"knowledge_pipeline_rag_dataset_index_ready":1`)) ||
		!bytes.Contains(response.Body.Bytes(), []byte(`"knowledge_pipeline_rag_dataset_index_missing_snapshot":1`)) ||
		!bytes.Contains(response.Body.Bytes(), []byte(`"knowledge_pipeline_rag_dataset_index_empty":1`)) ||
		!bytes.Contains(response.Body.Bytes(), []byte(`"knowledge_pipeline_rag_dataset_index_lagging":1`)) ||
...
func TestKnowledgePipelineDiagnosticsEndpointReturnsReadOnlyPipelines(t *testing.T) {
	mux := http.NewServeMux()
	httptrigger.RegisterKnowledgePipelineDiagnosticsRoutes(mux, staticKnowledgePipelineViewer{
		view: query.KnowledgePipelineDiagnosticsView{
			...
			Pipelines: []query.KnowledgePipelineView{{
				...
				RagDatasets: []query.KnowledgePipelineRagDatasetView{{
					...
					RagIndexState: query.KnowledgePipelineRagIndexStateView{
						Ready:           true,
						Status:          "ok",
						Reason:          "index_ready",
						MessageCount:    3,
						...
						LatestSourceSeq: 42,
						ParseRequested:  true,
						LastIngestAt:    "2026-05-31T10:00:00Z",
					},
					Status:  "warn",
	...
	response := httptest.NewRecorder()
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/v1/knowledge-pipeline-diagnostics?limit=10&stale_after_seconds=60", nil))
	...
	for _, expected := range []string{
		...
		`"rag_index_state":{"ready":true,"status":"ok","reason":"index_ready","message_count":3,"document_count":1,"start_seq":40,"end_seq":42,"latest_source_seq":42,"parse_requested":true,"last_ingest_at":"2026-05-31T10:00:00Z"}`,
		`"lag":20`,
		`"age_seconds":600`,
		`"side_effect":"none"`,
	} {
...
func TestMediaAssetEndpointRegistersListsAndServesContentRoute(t *testing.T) {
	...
	controlMutations := appservice.NewControlMutationAuditService()
	...
	cleanup := appservice.NewMediaAssetRetentionCleanupService(store, cleanupPreflight, controlMutations)
	mux := http.NewServeMux()
	httptrigger.RegisterRoutes(mux, ingestor, ingestor, shadowQueries, sender, imageJobs, outbox, mediaAssets, agentJobs, appservice.NewSendLedgerService(store), appservice.NewInboxEventService(store))
	httptrigger.RegisterMediaAssetRetentionCleanupRoutes(mux, cleanupPreflight, cleanup)
	...
	for _, expected := range []string{
		...
		`"/v1/media-assets/content-recovery-plan?asset_id=asset%3Aqq%3A1049511700%3Agroup%3A27234224%3Aqq%3Agqq%3A27234224%3A498%3A1"`,
		`"/v1/media-assets/content-recovery/preflight?asset_id=asset%3Aqq%3A1049511700%3Agroup%3A27234224%3Aqq%3Agqq%3A27234224%3A498%3A1"`,
	} {
		if !strings.Contains(bodyText, expected) {
	...
	cleanupBody := `{
		"asset_id": "` + assetID + `",
		"timestamp": "` + time.Now().UTC().Add(2*24*time.Hour).Format(time.RFC3339Nano) + `",
		"default_ttl_hours": 24,
		"target_id": "default-observed-group",
		"operator_id": "qsyy",
		"approval_id": "` + approval.ApprovalID + `",
		"mutation_id": "mutation-media-cleanup-http-1"
	}`
	response = httptest.NewRecorder()
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/v1/media-assets/retention-cleanup", strings.NewReader(cleanupBody)))
	if response.Code != http.StatusOK {
		t.Fatalf("expected retention cleanup 200, got %d: %s", response.Code, response.Body.String())
	}
	bodyText = response.Body.String()
	for _, expected := range []string{
		`"applied":true`,
		`"reason":"media_asset_retention_metadata_cleanup_applied"`,
		`"deleted_count":1`,
		`"deleted_asset_ids":["` + assetID + `"]`,
		`"status":"applied"`,
		`"side_effect":"runtime_state_cleanup"`,
	} {
		if !strings.Contains(bodyText, expected) {
			t.Fatalf("retention cleanup response missing %s: %s", expected, bodyText)
		}
	}

	response = httptest.NewRecorder()
	mux.ServeHTTP(response, httptest.NewRequest(http.MethodGet, "/v1/media-assets/retention-cleanup", nil))
	if response.Code != http.StatusMethodNotAllowed {
		t.Fatalf("expected retention cleanup 405, got %d: %s", response.Code, response.Body.String())
	}
}
...
type staticKnowledgePipelineViewer struct {
	view query.KnowledgePipelineDiagnosticsView
}
