...
func RegisterRoutes(
	...
	mux.Handle("/v1/media-assets/content-recovery-plan", MediaAssetContentRecoveryPlanHandler(mediaAssets))
...
func RegisterAgentJobCapacityRoutes(
	mux *http.ServeMux,
	planner inport.AgentJobCapacityPlanner,
) {
...
func RegisterControlMutationPolicyRoutes(
	mux *http.ServeMux,
	viewer inport.ControlMutationPolicyViewer,
) {
...
func ControlMutationPolicyHandler(viewer inport.ControlMutationPolicyViewer) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
			return
		...
		if viewer == nil {
			http.Error(w, "control mutation policy disabled", http.StatusNotImplemented)
			return
		}
		view, err := viewer.GetControlMutationPolicy(r.Context(), query.ControlMutationPolicyFilter{
			TargetKind: strings.TrimSpace(r.URL.Query().Get("target_kind")),
		})
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
...
func MediaAssetContentRecoveryPlanHandler(mediaAssets inport.MediaAssetManager) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodGet {
			http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
			return
		}
		view, err := mediaAssets.ContentRecoveryPlan(r.Context(), r.URL.Query().Get("asset_id"))
		if err != nil {
			http.Error(w, err.Error(), http.StatusBadRequest)
			return
		}
...
func AgentJobCapacityPlanHandler(planner inport.AgentJobCapacityPlanner) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if planner == nil {
			http.Error(w, "agent job capacity plan disabled", http.StatusNotImplemented)
			return
		}
		if r.Method != http.MethodGet {
			http.Error(w, "method not allowed", http.StatusMethodNotAllowed)
