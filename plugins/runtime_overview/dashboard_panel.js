...
(() => {
  ...
  function _target(kind, id) {
    const parts = [kind, id].map((value) => String(value ?? "").trim()).filter(Boolean);
    return parts.length ? parts.join(" / ") : "-";
  }
  function _renderMediaAssetContentDetail(detail) {
    const totals = _record(detail.totals);
    const items = _array(detail.items).slice(0, 20);
    const totalCells = ["assets", "ready", "forbidden", "unavailable", "disabled", "error"].map((key) => `
  ...
  function _renderMediaAssetRetentionDetail(detail) {
    ...
    const totalCells = ["assets", "cleanup_due", "default", "ephemeral", "permanent", "unknown"].map((key) => `
      <div class="runtime-overview-kpi">
        <span>${escapeHtml(key)}</span>
        <strong>${escapeHtml(String(totals[key] ?? 0))}</strong>
      </div>
  ...
  function _renderMediaAssetRetentionSteps(title, steps, emptyLabel) {
    const rows = steps.slice(0, 20).map((item) => {
      const metadata = _record(item.metadata);
      const metadataText = Object.keys(metadata).length ? Object.entries(metadata).map(([key, value]) => `${key}=${String(value ?? "")}`).join("\n") : "-";
      return `
      <tr>
  ...
  function _renderMediaAssetRetentionPlanDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(plan.reason || "-", 52))} \xB7 ${escapeHtml(String(plan.side_effect || "none"))}</div>
        </div>
      </div>
  ...
  function _renderMediaAssetRetentionCleanupDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(cleanup.reason || "-", 52))} \xB7 ${escapeHtml(String(cleanup.side_effect || "none"))}</div>
        </div>
        <div class="runtime-overview-links">
          ${_link(endpoints.plan, "plan")}
          ${_link(endpoints.preflight, "preflight")}
  ...
  function _renderDeadLettersDetail(detail) {
  ...
  function _renderCheckpointLagDetail(detail) {
    const diagnostics = _record(detail.diagnostics);
    const provided = _array(detail.checkpoints || detail.checkpoint_lag);
  ...
  `).join("");
    const lags = checkpoints.map((item) => Number(item.checkpoint_lag_messages ?? 0)).filter((value) => Number.isFinite(value));
  ...
  `).join("");
    const approvalKpis = ["approvals", "active", "approved", "rejected", "revoked"].map((key) => `
      <div class="runtime-overview-kpi">
        <span>${escapeHtml(key)}</span>
        <strong>${escapeHtml(String(approvalTotals[key] ?? 0))}</strong>
      </div>
    `).join("");
    const mutationKpis = ["mutations", "planned", "applied", "failed", "rolled_back"].map((key) => `
      <div class="runtime-overview-kpi">
        <span>${escapeHtml(key)}</span>
  ...
  function _renderRuntimeHealthDetail(detail) {
    const health = _record(detail.health || detail);
    const errors = Array.isArray(detail.errors) ? detail.errors : [];
    const healthRows = Object.entries(health).filter(([key]) => key !== "errors").map(([key, value]) => `
      <tr>
  ...
  function _renderStaleJobsDetail(detail) {
  ...
  function _renderJobEventsDetail(detail) {
  ...
  function _renderOutboxEventsDetail(detail) {
  ...
  `).join("");
    const recentRows = rowsSource.map((item) => {
      const channel = _record(item.channel);
      const channelLabel = [item.channel_kind || channel.kind, channel.account_id, channel.conversation_type].filter(Boolean).join(" / ");
      return `
      <tr>
  ...
  function _renderRagEvalFailuresDetail(detail) {
    const metrics = _record(detail.agent_job_metrics || detail);
    const deadLetters = _record(metrics.dead_letters);
    const byType = _record(deadLetters.by_type);
    const throughput = _record(metrics.throughput);
    const directItems = _array(detail.items);
    const fallbackItems = _array(deadLetters.recent).filter((item) => String(item.job_type || "") === "rag_eval");
    const rowsSource = (directItems.length ? directItems : fallbackItems).slice(0, 20);
    const notes = Array.isArray(detail.notes) ? detail.notes : Array.isArray(metrics.notes) ? metrics.notes : [];
    const rows = rowsSource.map((item) => {
      const result = _record(item.result);
      const passed = typeof item.passed === "boolean" ? item.passed : typeof result.passed === "boolean" ? result.passed : null;
      const lifecycleStatus = String(item.lifecycle_status || item.status || "-");
      const qualityStatus = String(item.quality_status || (passed === false ? "failed_quality" : "-"));
      const errorLabel = String(item.error_message || item.coverage_reason || qualityStatus || "-");
      return `
      <tr>
  ...
  function _renderRuntimeConfigDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(runtime.address || "-", 48))} \xB7 ${escapeHtml(_short(runtime.address_source || "-", 28))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
  ...
  function _renderWorkerLeasesDetail(detail) {
  ...
  function _renderQqCutoverRouteTable(title, routes, emptyLabel) {
  ...
  function _renderQqCutoverRouteMatrixDetail(detail) {
    ...
    const configuredBotIds = Array.isArray(qqAccounts.configured_bot_ids) ? qqAccounts.configured_bot_ids.map((item) => String(item ?? "")) : [];
    return `
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
  ...
  function _renderKnowledgeJobPlannerPreviewDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(String(preview.timestamp || "-"), 44))} \xB7 bucket ${escapeHtml(String(preview.bucket ?? "-"))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
  ...
  function _renderKnowledgeJobPlannerReadinessDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(readiness.reason || "-", 44))} \xB7 ${escapeHtml(String(readiness.side_effect || "none"))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
  ...
  function _renderAgentJobExternalLeaseSteps(title, steps) {
    const rows = steps.slice(0, 20).map((item) => {
      const env = _record(item.env);
      const envText = Object.keys(env).length ? Object.entries(env).map(([key, value]) => `${key}=${String(value ?? "")}`).join("\n") : "-";
      return `
      <tr>
  ...
  function _renderExternalLeaseDiagnosticsDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(queue.migration_phase || "-", 24))} \xB7 ${escapeHtml(_short(queue.lease_owner || "-", 28))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
  ...
  function _renderSchedulerJobsDetail(detail) {
  ...
  function _renderAgentWorkersDetail(detail) {
  ...
  function _renderRuntimeWorkersDetail(detail) {
  ...
  function _renderAgentJobWorkerCoverageDetail(detail) {
  ...
  function _renderAgentJobPressureDetail(detail) {
  ...
  function _renderAgentJobMetricsDetail(detail) {
    const metrics = _record(detail.agent_job_metrics || detail);
    const throughput = _record(metrics.throughput);
    const deadLetters = _record(metrics.dead_letters);
  ...
  function _renderOutboxPressureDetail(detail) {
  ...
  function _renderOutboxMetricsDetail(detail) {
    const metrics = _record(detail.outbox_metrics || detail.outbox_pressure || detail);
    const throughput = _record(metrics.throughput);
    const deadLetters = _record(metrics.dead_letters);
    const byChannelKind = _record(metrics.deliveries_by_channel_kind);
  ...
  function _renderSendLedgerMetricsDetail(detail) {
  ...
  function _renderInboxMetricsDetail(detail) {
    const metrics = _record(detail.inbox_metrics || detail);
    const byChannelKind = _record(metrics.events_by_channel_kind);
    const byConversation = Object.entries(_record(metrics.events_by_conversation)).slice(0, 20).map(([conversationKey, rawValue]) => {
      const item = _record(rawValue);
      const channel = _record(item.channel);
      const channelLabel = [channel.kind, channel.account_id, channel.conversation_type].filter(Boolean).join(" / ");
      return `
        <tr>
    ...
    const recentRows = _array(metrics.recent).slice(0, 20).map((item) => {
      const channel = _record(item.channel);
      const channelLabel = [channel.kind, channel.account_id, channel.conversation_id].filter(Boolean).join(" / ");
      return `
      <tr>
  ...
  function _renderInboundDedupeMetricsDetail(detail) {
  ...
  function _renderObserveTargetsDetail(detail) {
  ...
  function _renderObserveCaptureDetail(detail) {
  ...
  function _renderDeliveryAdaptersDetail(detail) {
  ...
  function _renderAgentJobExternalLeaseReadinessDetail(detail) {
  ...
  function _renderAgentJobExternalLeasePlanDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(plan.decision || "-", 24))} \xB7 ${escapeHtml(String(plan.side_effect || "none"))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
        <div class="runtime-overview-kpi"><span>ready</span><strong>${escapeHtml(String(Boolean(plan.ready)))}</strong></div>
  ...
  function _renderRuntimeOverviewNotesSection(title, lines, emptyLabel) {
  ...
  function _renderDeliverySmokeReadinessDetail(detail) {
  ...
  function _renderQueueBackendDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(`${String(backend.provider || "-")} / ${String(backend.mode || "-")}`, 40))} \xB7 ${escapeHtml(_short(backend.migration_phase || "-", 24))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
  ...
  function _renderReceiverStatusesDetail(detail) {
  ...
  function _renderReceiverLeasesDetail(detail) {
  ...
  function _renderKnowledgePipelinesDetail(detail) {
    ...
    const rows = pipelines.map((item) => {
      ...
      const coverageStatuses = coverage.map((coverageItem) => _short(coverageItem.coverage_status || coverageItem.coverage_reason || "-", 18)).filter(Boolean).join(", ");
      const reasons = [
  ...
  function _renderAgentJobCapacityPlanDetail(detail) {
    ...
    <div class="runtime-overview-section">
      <div class="runtime-overview-section-header">
        <div>
          ...
          <div class="detail-subtext">${escapeHtml(_short(plan.reason || "-", 44))} \xB7 ${escapeHtml(String(plan.side_effect || "none"))}</div>
        </div>
      </div>
      <div class="runtime-overview-kpis">
        <div class="runtime-overview-kpi"><span>ready</span><strong>${escapeHtml(String(Boolean(plan.ready)))}</strong></div>
  ...
  function _renderAgentJobPriorityPlanDetail(detail) {
  ...
  function _renderKnowledgePlannerCutoverPlanDetail(detail) {
  ...
  function _renderOutboundCutoverPlanDetail(detail) {
  ...
  window.AkashicDashboard.registerPlugin({
    ...
    renderDetail(item, container) {
      ...
      ` : "";
      const specializedDetail = card.id === "delivery_smoke" ? _renderDeliverySmokeReadinessDetail(_record(card.detail)) : card.id === "delivery_adapters" ? _renderDeliveryAdaptersDetail(_record(card.detail)) : card.id === "queue_backend" ? _renderQueueBackendDetail(_record(card.detail)) : card.id === "receiver_statuses" ? _renderReceiverStatusesDetail(_record(card.detail)) : card.id === "receiver_leases" ? _renderReceiverLeasesDetail(_record(card.detail)) : card.id === "knowledge_pipelines" ? _renderKnowledgePipelinesDetail(_record(card.detail)) : card.id === "media_asset_retention_plan" ? _renderMediaAssetRetentionPlanDetail(_record(card.detail)) : card.id === "media_asset_retention_cleanup" ? _renderMediaAssetRetentionCleanupDetail(_record(card.detail)) : card.id === "dead_letters" ? _renderDeadLettersDetail(_record(card.detail)) : card.id === "checkpoint_lag" ? _renderCheckpointLagDetail(_record(card.detail)) : card.id === "job_events" ? _renderJobEventsDetail(_record(card.detail)) : card.id === "outbox_events" ? _renderOutboxEventsDetail(_record(card.detail)) : card.id === "rag_eval_failures" ? _renderRagEvalFailuresDetail(_record(card.detail)) : card.id === "runtime_health" ? _renderRuntimeHealthDetail(_record(card.detail)) : card.id === "stale_jobs" ? _renderStaleJobsDetail(_record(card.detail)) : card.id === "knowledge_job_planner_preview" ? _renderKnowledgeJobPlannerPreviewDetail(_record(card.detail)) : card.id === "knowledge_job_planner_readiness" ? _renderKnowledgeJobPlannerReadinessDetail(_record(card.detail)) : card.id === "media_asset_content" ? _renderMediaAssetContentDetail(_record(card.detail)) : card.id === "media_asset_content_recovery" ? _renderMediaAssetContentRecoveryDetail(_record(card.detail)) : card.id === "media_asset_retention" ? _renderMediaAssetRetentionDetail(_record(card.detail)) : card.id === "worker_leases" ? _renderWorkerLeasesDetail(_record(card.detail)) : card.id === "queue_topology" ? _renderQueueTopologyDetail(_record(card.detail)) : card.id === "qq_cutover_route_matrix" ? _renderQqCutoverRouteMatrixDetail(_record(card.detail)) : card.id === "external_lease_diagnostics" ? _renderExternalLeaseDiagnosticsDetail(_record(card.detail)) : card.id === "scheduler_jobs" ? _renderSchedulerJobsDetail(_record(card.detail)) : card.id === "agent_workers" ? _renderAgentWorkersDetail(_record(card.detail)) : card.id === "runtime_workers" ? _renderRuntimeWorkersDetail(_record(card.detail)) : card.id === "observe_targets" ? _renderObserveTargetsDetail(_record(card.detail)) : card.id === "observe_capture" ? _renderObserveCaptureDetail(_record(card.detail)) : card.id === "agent_job_worker_coverage" ? _renderAgentJobWorkerCoverageDetail(_record(card.detail)) : card.id === "agent_job_metrics" ? _renderAgentJobMetricsDetail(_record(card.detail)) : card.id === "agent_job_pressure" ? _renderAgentJobPressureDetail(_record(card.detail)) : card.id === "send_ledger_metrics" ? _renderSendLedgerMetricsDetail(_record(card.detail)) : card.id === "inbox_metrics" ? _renderInboxMetricsDetail(_record(card.detail)) : card.id === "inbound_dedupe_metrics" ? _renderInboundDedupeMetricsDetail(_record(card.detail)) : card.id === "outbox_metrics" ? _renderOutboxMetricsDetail(_record(card.detail)) : card.id === "outbox_pressure" ? _renderOutboxPressureDetail(_record(card.detail)) : card.id === "agent_job_external_lease_readiness" ? _renderAgentJobExternalLeaseReadinessDetail(_record(card.detail)) : card.id === "agent_job_external_lease_plan" ? _renderAgentJobExternalLeasePlanDetail(_record(card.detail)) : card.id === "agent_job_capacity_plan" ? _renderAgentJobCapacityPlanDetail(_record(card.detail)) : card.id === "agent_job_priority_plan" ? _renderAgentJobPriorityPlanDetail(_record(card.detail)) : card.id === "knowledge_job_planner_cutover_plan" ? _renderKnowledgePlannerCutoverPlanDetail(_record(card.detail)) : card.id === "outbound_cutover_plan" ? _renderOutboundCutoverPlanDetail(_record(card.detail)) : card.id === "control_audit" ? _renderControlAuditDetail(_record(card.detail)) : card.id === "control_mutation_policy" ? _renderControlMutationPolicyDetail(_record(card.detail)) : card.id === "runtime_config" ? _renderRuntimeConfigDetail(_record(card.detail)) : "";
      container.innerHTML = `
